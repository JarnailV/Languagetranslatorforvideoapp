...
def decode_sequence(input_seq):
    ...
    target_seq = np.array(
        [co.transform(list("\t")).toarray().tolist()], dtype="float32")
    ...
    while not stop_condition:
        ...
        output_chars, h, c = dec_model.predict(
...
def bagofcharacters(input_t):
    ...
    num_en_chars = datafile['num_en_chars']
    num_dec_chars = datafile['num_dec_chars']
    cv = CountVectorizer(binary=True, tokenizer=lambda txt: txt.split(
    ), stop_words=None, analyzer='char')
    en_in_data = []
...
def deocded_output(msg):
    ...
    return str(decode_sequence(en_in_data))
    

txt = '''Your text here Jr Intermediate with 50% marks He for Mr participate serving in Army soldier & Students that belonging different areas like: FATA, Dir., Chitral, Gilgit Baltistan, District Neelam Azad Jammu Kashmir, Ten. Bala tot, Salamar, Mojgarh, Drawer fort.'''


def split(string):
    words = string.split()
    grouped_words = [' '.join(words[i: i + 8]) for i in range(0, len(words), 8)]
